...
run_result = {key1: {key: dict(acer=100, apcer=1, bpcer=1, run=None, batch_size=0, lambda_NCE=0) for key in ds} for key1
              in ds}
    ...
    for run in api.runs(f"{train}"):
        ...
        for test in ds:
            if summary[f"acer/{test}/TEST"]["min"] * 100 < run_result[train][test]['acer']:
                ...
                run_result[train][test]['batch_size'] = run.config['CLASSIFIER']['equiv_batch_size']
                run_result[train][test]['lambda_NCE'] = run.config['CUT']['lambda_NCE']
