        ...
        for name, loader in loaders.train.items():
            if (not loader.config.skip and not warmup) or warmup:
                if not loader.config.selftraing:
                    ...
                    if self_training:
                        if warmup or not loader.config.skip:
                            ...
                            paths_for_selftraining.label_0 = vocab.index2word(paths_for_selftraining.label_0)
                            paths_for_selftraining.label_1 = vocab.index2word(paths_for_selftraining.label_1)
                            for batch in _make_data_loader(args,
                                                           accelerator, vocab, **paths_for_selftraining,
